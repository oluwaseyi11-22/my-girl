...
function createHeart(){
    ...
    if (!heartContainer || isTouchDevice || window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
...
if (showLetter && letter) {
	showLetter.addEventListener("click", () => {
		...
		showLetter.innerHTML = isOpen ? "Hide Letter ❤️" : "Open My Letter 💌";
		showLetter.setAttribute("aria-expanded", String(isOpen));

		if (isOpen) {
			letter.querySelector('.letter-fold')?.remove();
			letter.insertAdjacentHTML('beforeend', '<div class="letter-fold"></div>');
			letter.insertAdjacentHTML('beforeend', '<span class="letter-heart letter-heart-1">💗</span><span class="letter-heart letter-heart-2">✨</span><span class="letter-heart letter-heart-3">🌟</span>');
			setTimeout(() => {
				letter.querySelectorAll('.letter-heart').forEach((el) => el.remove());
			}, 1600);
		} else {
			letter.querySelector('.letter-fold')?.remove();
		}
